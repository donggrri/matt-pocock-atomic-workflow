...
3. Write `.docs/<slug>/PLAN-<slug>.md` for product work using the skill template. Keep the `PREFIX-<slug>.md` filename. Use codebase-design language (module, interface, seam, depth) in the design section when code is involved.
...
If this request is to inspect or edit matt-pocock-atomic-workflow itself (skills, commands, agents, packages), write `~/.pi/agent/matt-pocock-atomic-workflow/docs/<slug>/PLAN-<slug>.md` (Cursor: `~/.cursor/matt-pocock-atomic-workflow/docs/<slug>/`). Never mix product root, skill folders, and home docs. Do not skip writing the PLAN for workflow meta-review.
