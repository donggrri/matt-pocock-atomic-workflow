...
Cursor 세션이다. 남은 구현이 있으면 기본 파이프라인으로 그 항목부터 돌린 다음 리뷰한다. 그 외에는 Task 툴로 `reviewer`를 백그라운드로 띄운다. task 첫 줄에 `matt-pocock-atomic-workflow`·`code-review` 경로를 적는다. 손자 에이전트를 시키라고 하지 마라. 산출물은 여전히 `.docs/<slug>/REVIEW-<slug>.md`(워크플로 자체는 `docs/<slug>/`)다.
