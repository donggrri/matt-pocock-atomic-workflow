...
`run-done` 게이트: 모든 `done` 명령은 부모가 `scripts/run-done.mjs`로 재실행하고 `.done.json` 증거를 확인해야 통과로 인정된다. 워커의 「테스트 통과」 문장만으로는 충분하지 않다.
...
워커 브리프에도 같은 `done` 명령을 넣는다. 워커의 「테스트 통과」문장만 믿지 않는다. `run-done`으로 `.done.json` 증거가 있어야 통과로 인정한다.
