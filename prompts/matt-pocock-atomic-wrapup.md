---
description: matt-pocock-atomic-workflow Phase 5 wrapup. 커밋하고 전역 상태를 갱신한다. 푸시 없음.
...
5. 푸시하지 않는다.
6. 커밋이 성공하면 전역 상태를 갱신한다. `node scripts/work-status.mjs sync <slug> commit`. 사용자가 「완료」라고 하면 `node scripts/work-status.mjs sync <slug> complete`(또는 `complete <slug>`)로 `phase: complete`를 기록한다. 푸시만으로는 완료가 되지 않는다 (Cursor 설치: `.agents/skills/matt-pocock-atomic-workflow/scripts/work-status.mjs`).
