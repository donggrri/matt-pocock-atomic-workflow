...
---
`atomic-workflow` 스킬을 읽고 Phase 3을 수행한다. 「구현만」이 아니면 열린 항목이 끝난 뒤 `g-reviewer`까지 자동 진행한다.
...
- 그 외 구현 항목은 `g-worker`를 `async: true`로. task 첫 줄에 `atomic-workflow`·`tdd` 경로를 적는다. 로직은 `tdd`를 강제한다.
