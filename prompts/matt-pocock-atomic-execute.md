...
Pi 세션이다. `agy`/`pi`/`codex` CLI를 직접 호출하지 마라. 위임은 `subagent`만 쓴다.
...
- 인자가 `agy` / `flash` / `pro` / `sonnet`이면 `worker`를 해당 Antigravity 모델로 `async: true`로 띄운다 (`antigravity/gemini-3-8-flash:high`, `antigravity/gemini-3-1-pro:high`, `antigravity/claude-sonnet-4-6`).
- 그 외 구현 항목은 `worker`를 `async: true`로. task 첫 줄에 `matt-pocock-atomic-workflow`·`tdd` 경로를 적는다. 로직은 `tdd`를 강제한다.
- 항목마다 구현 후 **부모가 `run-done`으로 증거를 확인하고 통과할 때만** `[x]` → `node scripts/work-status.mjs sync <slug>`. 실패면 `막힘:`과 로그를 남기고 `sync <slug>` 후 멈춘다 (Cursor 설치: `.agents/skills/matt-pocock-atomic-workflow/scripts/work-status.mjs`).
