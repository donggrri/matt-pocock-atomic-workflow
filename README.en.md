...
Replace `YOUR_*` placeholders with real model IDs. Example:
...
/login xai
/login cursor
agy          # interactive auth once (not needed after that)
```
...
The easy way is to run `/matt-pocock-atomic-config <agent> <model>` in a Pi session, or `/matt-pocock-atomic-config` for the interactive flow.
